...
export function hasExecutionBlock(text) {
  ...
  // a ±100-character window of the alias match. The bidirectional window
  // matters because real execution blocks place the underline either
  // BEFORE the alias (e.g. signature line above "Witness Name:") or
  // AFTER it (e.g. underscores below "SIGNED by [Party]"). Body prose
  // false positives like "amendment signed by authorised representatives"
  // are blocked by the alias regex itself (uppercase + line-start
  // requirement on SIGNED) before this check runs.
  for (const alias of EXECUTION_BLOCK_ALIASES) {
    const match = alias.exec(tail)
    if (!match) continue
    const start = Math.max(0, match.index - 100)
    const end = Math.min(tail.length, match.index + match[0].length + 100)
    const window = tail.slice(start, end)
    if (EXECUTION_BLOCK_STRUCTURAL_ANCHORS.some(a => a.test(window))) return true
  }

  // Multi-line fallback (preserved): bare 8+-underscore line followed by
  // a "Date:" line within the next 3 lines. Threshold lowered from 10 to
  // 8 to align with the structural-anchor regex above.
  const lines = tail.split('\n')
  for (let i = 0; i < lines.length; i++) {
    if (/_{8,}/.test(lines[i])) {
      const window = lines.slice(i, Math.min(i + 4, lines.length))
      if (window.some(l => /\bdate\s*:/i.test(l))) return true
    }
  }
  return false
}
