 ...
 * signature-block markers PAIRED WITH a structural anchor. Scoping to the
 * tail avoids most false positives from clause headings that mention
 * "signatures" in the body; the structural-pairing requirement avoids the
 * remaining false-positive class where body prose in the tail (e.g.
 * "...amendment signed by authorized representatives...") would have
 * matched a substring-only detector.
 *
 * Customer-impact regression that motivated this design — SIG-858825A9
 * (2026-05-07) — see fix(completeness) commit history. The previous
 * implementation used case-insensitive substring matches with no
 * structural pairing, which let a Section 3.9 amendment clause stamp a
 * truncated MOU as `ok: true` and bypass the continuation retry.
 *
 * Returns true when the execution block is present, false when absent.
 *
 * Aliases (any one suffices, but each must be paired with a structural
 * anchor within the next 200 characters):
 *   1. /^[\s>*-]*SIGNED\s+(by|on\s+behalf\s+of)\b/m — uppercase SIGNED at
 *      line start. EXECUTION_FORMALITIES_CLAUSE templates produce this
 *      capitalised, line-anchored form. Body prose like "...signed by
 *      authorised representatives..." is lowercase and mid-sentence and
 *      will not match.
 *   2. /\bIN\s+WITNESS\s+WHEREOF\b/i — formal preamble. Practically never
 *      occurs in body prose; case-insensitive is safe.
 *   3. /\bWitness\s+(Name|Signature)\s*:/i — colon-suffixed form-field
 *      marker. The trailing colon distinguishes it from any body usage of
 *      "witness name" without colon.
 *
 * Structural anchors (any one of these must appear within 200 chars of the
 * matched alias):
 *   - 8+ contiguous underscores (signature underline)
 *   - "Date:" form field
 *   - "Print Name:" / "Printed Name:" / "Printed by:" form field
 *   - "Title:" form field
 *
 * Multi-line fallback (preserved from earlier fix): an 8+-underscore line
 * paired with a "Date:" line within the next 3 lines also counts. This
 * supports jurisdictions that emit a bare signature underline + date line
 * without using any of the keyword aliases above.
 */
const EXECUTION_BLOCK_ALIASES = [
  /^[\s>*-]*SIGNED\s+(by|on\s+behalf\s+of)\b/m,
  /\bIN\s+WITNESS\s+WHEREOF\b/i,
  /\bWitness\s+(Name|Signature)\s*:/i,
]

const EXECUTION_BLOCK_STRUCTURAL_ANCHORS = [
  /_{8,}/,
  /\bDate\s*:/i,
  /\b(Print|Printed)\s+(Name|by)\s*:/i,
  /\bTitle\s*:/i,
]

export function hasExecutionBlock(text) {
  const str = text || ''
  if (str.length < 100) return false

  const tail = str.slice(Math.floor(str.length * 0.7))

  // Alias + structural-pairing path. Each alias must be paired with a
  // structural anchor (underline / Date: / Print Name: / Title:) within
  // a -100/+400 character window of the alias match. Asymmetric because:
  //   - "Witness Name:" / "Witness Signature:" aliases sometimes appear
  //     AFTER the signature underline, so we need a small look-back window.
  //   - "IN WITNESS WHEREOF" / "SIGNED by" aliases lead the execution
  //     block; the structural fields follow over the next several
  //     hundred chars (preamble + per-party blocks). +400 covers the
  //     typical layout (alias + 1-2 sentence preamble + at least one
  //     party's signature lines).
  // Body prose false positives like "amendment signed by authorised
  // representatives" are blocked by the alias regex itself (uppercase +
  // line-start requirement on SIGNED) before this check runs.
  for (const alias of EXECUTION_BLOCK_ALIASES) {
    const match = alias.exec(tail)
    if (!match) continue
    const start = Math.max(0, match.index - 100)
    const end = Math.min(tail.length, match.index + match[0].length + 400)
    const window = tail.slice(start, end)
    if (EXECUTION_BLOCK_STRUCTURAL_ANCHORS.some(a => a.test(window))) return true
  }

  // Multi-line fallback (tightened for SIG-57B913C5 regression). The previous
  // form (underscore line + "Date:" within 3 lines) leaked on short documents
  // where Sonnet wrote a body clause like "Section 7. Effective Date and Term
  // — The Effective Date shall be ____________________ … Date: ____________"
  // embedded in the tail. That looked structurally identical to a minimal
  // execution block but was actually body content with no signature semantics.
  //
  // Tightened rule: underscore line + "Date:" line in proximity is no longer
  // sufficient. The window must ALSO contain at least one execution-block-
  // specific form-field marker — Title: / Print Name: / Printed Name: /
  // Printed by: / Signature:. These labels appear in real exec blocks
  // (EXECUTION_FORMALITIES_CLAUSE template instructs Sonnet to emit them)
  // but essentially never in body prose like "Effective Date" or "Term"
  // clauses.
  //
  // Trade-off acknowledged: minimal exec block styles that omit all four
  // labels (e.g. "For [Party]: ___ / Date: ___" with nothing else) are now
  // false-negatives and will trigger the orchestrator's continuation retry.
  // That's the better failure mode — retry produces a re-prompted output
  // that almost always emits the labelled form. Better false-negative
  // (retry overhead) than false-positive (broken doc stamps ok:true).
  const STRICT_FORM_FIELDS = [
    /\b(Print|Printed)\s+(Name|by)\s*:/i,
    /\bTitle\s*:/i,
    /\bSignature\s*:/i,
  ]
  ...
  for (let i = 0; i < lines.length; i++) {
    if (!/_{8,}/.test(lines[i])) continue
    const window = lines.slice(i, Math.min(i + 6, lines.length))
    const hasDate = window.some(l => /\bdate\s*:/i.test(l))
    if (!hasDate) continue
    if (window.some(l => STRICT_FORM_FIELDS.some(re => re.test(l)))) return true
  }
  return false
}
