...
export async function sendReceipt({ to, subject, html, invoiceId }) {
  ...
  // Default FROM uses getsignova.com which is Resend-verified and reply-safe.
  // Override with RESEND_FROM_ADDRESS env var when sending Ebenova-branded
  // API receipts (requires ebenova.dev to be verified in Resend first).
  const from = process.env.RESEND_FROM_ADDRESS || 'Signova <noreply@getsignova.com>'
