...
export default async function handler(req, res) {
  ...
  const nigeriaTenancyClause = (isTenancyDoc || isQuitNoticeDoc || isLandlordAgentDoc) && isNigeria
    ? '\n\nIMPORTANT — NIGERIAN TENANCY LAW: Apply the following statutes and principles for Nigerian residential and commercial tenancies:\n' +
      ...
      '(iv) CAUTION / SECURITY DEPOSIT — Must be held separately with a separate receipt issued on collection. LSTL s.10 requires the landlord to provide a written account of how the deposit has been applied at least every six (6) months. Must be refunded within a reasonable time after the tenancy ends, minus only itemised deductions for unpaid rent or damage beyond fair wear and tear, with each deduction accounted for in writing.\n' +
      ...
      '(viii) RENT RECEIPTS — LSTL s.5 requires the landlord to issue a receipt for every rent payment stating: date of receipt, names and addresses of landlord and tenant, description of premises, amount paid, and period covered. Include a clause requiring this.\n' +
      '(ix) RENT INCREASE — Any rent increase must be reasonable (LSTL s.37, which gives the tenant the right to challenge excessive increases in court). The landlord must give at least 6 months\' written notice before the current lease expires, and increases may not be more frequent than once every 2 years. Note that Lagos State is considering the Lagos State Tenancy Bill 2025, which may introduce tighter restrictions; generated agreements should accommodate future amendments.'
    : ''
